...
class Users extends Controller
{
	...
	{
		$newrecord_problemid="";
		$getproblem_data=$this->ProblemsModel->where('type',"第一幕")->limit(2)->orderRaw('rand()')->select();
		for($i=0;$i<2;++$i)
		{
			$return_data[$i]=$getproblem_data[$i];
			$newrecord_problemid = $newrecord_problemid . $getproblem_data[$i]['id'] . ",";
		}
		$getproblem_data=$this->ProblemsModel->where('type',"第二幕")->limit(3)->orderRaw('rand()')->select();
		for($i=0;$i<3;++$i)
		{
			$return_data[$i+3]=$getproblem_data[$i];
			$newrecord_problemid = $newrecord_problemid . $getproblem_data[$i]['id'] . ",";
		}
		$getproblem_data=$this->ProblemsModel->where('type',"第三幕")->limit(3)->orderRaw('rand()')->select();
		for($i=0;$i<3;++$i)
		{
			$return_data[$i+3]=$getproblem_data[$i];
			$newrecord_problemid = $newrecord_problemid . $getproblem_data[$i]['id'] . ",";
		}
		$getproblem_data=$this->ProblemsModel->where('type',"第四幕")->limit(3)->orderRaw('rand()')->select();
		for($i=0;$i<2;++$i)
		{
  ...
  //       return $info;
	...
	function getvedio()
	{
		if (!empty($_GET["num"]) && 0<$_GET["num"] && $_GET["num"]<=4) {
			return $_SERVER['HTTP_HOST'].'/video/video'.$_GET["num"].'.mp4';
		}
		else{
			return 'error';
		}
